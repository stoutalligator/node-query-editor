...
function tokenize(src) {
  ...
  while (i < src.length) {
    ...
    if (src[i] === ">") {
      tokens.push({ kind: "GT", value: ">", pos: start });
      i++;
      continue;
    }
    if (src[i] === "<") {
      tokens.push({ kind: "LT", value: "<", pos: start });
      i++;
      continue;
    }
    if (src[i] === ",") {
      tokens.push({ kind: "COMMA", value: ",", pos: start });
      i++;
      continue;
    }
    if (src[i] === "(") {
    ...
    if (src[i] === "*") {
      tokens.push({ kind: "STAR", value: "*", pos: start });
      i++;
      continue;
    }
    if (src[i] === ".") {
...
function parse(src) {
  ...
  if (ts.peekKw("XPATH")) {
    ...
    return { kind: "xpath", expression: rest };
  }
  if (ts.peekKw("WITH")) {
    return parseCte(ts);
  }
...
function parseWhere(ts) {
  ts.expectKw("WHERE");
  return parseOrExpr(ts);
}
function parseOrExpr(ts) {
  const clauses = [parseAndExpr(ts)];
  while (ts.consumeKw("OR")) {
    clauses.push(parseAndExpr(ts));
  }
  return clauses.length === 1 ? clauses[0] : { kind: "or", clauses };
}
function parseAndExpr(ts) {
  const clauses = [parsePrimary(ts)];
...
function parseCte(ts) {
  ...
  if (ts.consumeKw("HAVING")) {
    ...
    const opTok = ts.next();
    const valTok = ts.next();
    having = { op: opTok.value, value: parseFloat(valTok.value) };
  }
  let limit = null;
  if (ts.peekKw("LIMIT")) {
    ts.next();
    limit = parseInt(ts.next().value, 10);
  }
  return { kind: "cte", ctes, final: { columns, from: fromTable, fromAlias, joins, groupBy, having, limit } };
}
function parseColRef(ts) {
  const parts = [];
...
function loadXmlForPath(xml, filePath) {
  docCache.set(filePath, parseDocument(xml, { xmlMode: true }));
}
function hasDocForPath(filePath) {
  return docCache.has(filePath);
}
...
function runDirExtract(query) {
  const dirPath = query.source.path;
  const fileNames = fs.readdirSync(dirPath).filter((f) => f.toLowerCase().endsWith(".xml")).sort();
  const allRows = [];
  for (const fileName of fileNames) {
    ...
    const fileQuery = { ...query, source: { kind: "file", path: fp } };
...
function runExtract(q, doc, limitOverride) {
  const limit = limitOverride ?? q.limit;
  const rows = [];
  const ancestorAttrs = {};
  descend(q.steps, 0, doc, ancestorAttrs, {}, q.select, rows, limit);
  const columns = deriveColumns(q.select, rows);
  return { columns, rows, totalRows: rows.length, truncated: false };
}
function descend(steps, stepIdx, context, ancestorAttrs, ancestorNodes, select, out, limit) {
...
function resolvePathContext(path3, ancestorNodes, defaultContext) {
  const m = path3.match(/^([a-zA-Z_]\w*)\//);
  if (m && ancestorNodes[m[1]]) return ancestorNodes[m[1]];
  return defaultContext;
}
function matchPath(path3, context) {
  const normalized = path3.replace(/^[a-zA-Z_]\w*\/\//, "//");
  ...
  for (const part of parts) {
    ...
    for (const node of current) {
      ...
      for (const c of children) {
        if (isTag2(c) && (part === "*" || c.name === part)) next.push(c);
      }
    }
    current = next;
  }
  return current.filter(isTag2);
}
function findDescendants(node, tagName) {
  const results = [];
  const stack = getChildren(node).slice();
  while (stack.length) {
    const cur = stack.pop();
    if (isTag2(cur)) {
      if (tagName === null || cur.name === tagName) results.push(cur);
...
function getChildren(node) {
  if (isDocument(node)) return node.children;
  if (isTag2(node)) return node.children;
  return [];
}
function passesWhere(el, where) {
  ...
  switch (where.op) {
    ...
    case "!=":
      return val !== where.value;
    case ">":
      return parseFloat(val) > parseFloat(where.value);
    case "<":
      return parseFloat(val) < parseFloat(where.value);
    case ">=":
      return parseFloat(val) >= parseFloat(where.value);
    case "<=":
      return parseFloat(val) <= parseFloat(where.value);
    case "IN":
      return where.value.includes(val);
    case "NOT IN":
      return !where.value.includes(val);
  }
}
...
function buildRow(select, ancestorAttrs, ancestorNodes, leafEl) {
  const row = {};
...
function deriveColumns(select, rows) {
...
function runCte(q, limitOverride) {
  const tables = {};
  ...
  for (const join3 of final.joins) {
    const right = tables[join3.table] ?? [];
    rows = applyJoin(rows, final.fromAlias, right, join3);
  }
  if (final.groupBy && final.groupBy.length > 0) {
    const collectExprs = final.columns.map((c) => c.expr).filter((e) => /^COLLECT\((.+)\)$/.test(e)).map((e) => {
      const inner = e.match(/^COLLECT\((.+)\)$/)[1];
      const dirMatch = inner.match(/^(.+?)\s+(ASC|DESC)$/i);
      return { full: e, col: dirMatch ? dirMatch[1] : inner, dir: dirMatch ? dirMatch[2].toUpperCase() : null };
    });
    ...
    for (const { row, count, collected } of groups.values()) {
      ...
      for (const [expr, vals] of collected) {
        ...
        if (ce.dir) {
          ...
          sorted = allNumeric ? [...vals].sort((a, b) => ce.dir === "ASC" ? Number(a) - Number(b) : Number(b) - Number(a)) : [...vals].sort((a, b) => ce.dir === "ASC" ? a.localeCompare(b) : b.localeCompare(a));
        }
...
function applyJoin(left, leftAlias, right, join3) {
  const idx = /* @__PURE__ */ new Map();
  for (const rr of right) {
    const key = join3.on.map((c) => resolveColExpr(c.right, rr)).join("\0");
    if (!idx.has(key)) idx.set(key, []);
    idx.get(key).push(rr);
  }
  const out = [];
  for (const lr of left) {
    const key = join3.on.map((c) => resolveColExpr(c.left, lr)).join("\0");
    const matches = idx.get(key);
    if (matches && matches.length > 0) {
      for (const rr of matches) {
        const merged = {};
        for (const [k, v] of Object.entries(lr)) merged[`${leftAlias}.${k}`] = v;
        for (const [k, v] of Object.entries(rr)) merged[`${join3.alias}.${k}`] = v;
        out.push(merged);
      }
    } else if (join3.type === "LEFT") {
      const merged = {};
      for (const [k, v] of Object.entries(lr)) merged[`${leftAlias}.${k}`] = v;
      out.push(merged);
    }
...
function applyHaving(count, having) {
  switch (having.op) {
    case ">":
...
// src/cli.ts
function runDirQuery(query, dirPath) {
  let fileNames;
  try {
    fileNames = fs2.readdirSync(dirPath).filter((f) => f.toLowerCase().endsWith(".xml")).sort();
  } catch (err) {
  ...
  for (const fileName of fileNames) {
    ...
    const fp = path2.join(dirPath, fileName);
    if (!hasDocForPath(fp)) {
      loadXmlForPath(fs2.readFileSync(fp, "utf8"), fp);
    }
...
`);
  ...
  if (spec.xmlFile) {
    try {
      loadXml(fs2.readFileSync(spec.xmlFile, "utf8"), spec.xmlFile);
    } catch (err) {
...
`);
  ...
  for (const { name, query: queryText } of spec.queries) {
    const substituted = queryText.replace(/\{xmlFile\}/g, spec.xmlFile ?? "").replace(/\{xmlDir\}/g, spec.xmlDir ?? "");
    let parsed;
    try {
